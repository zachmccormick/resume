...
%          {Enhanced messaging pipeline behaviors such as time and volume-based rate limiting, quiet time parameters, enhanced send time optimization, campaign threshold alerting, etc.}
%        \resumeItem{Local Development Environment Rebuild}
%          {Rebuilt local development environment for all engineers using state-of-the-art Docker and Docker Compose to decrease compatibility issues, increase performance, and decrease customization.}
...
%        \resumeItem{Internal Documentation Reboot}
%          {Provided initial spark to form an Information Committee, responsible for organizating and improving our source of internal technical documentation.}
%        \resumeItem{Led Platform Backend Committee}
%          {After the product organization broke into product vertical teams, I led the Platform Backend Committee - a cross-team effort to address technical debt and components with less-clear ownership.}
%        \resumeItem{Revamp of Frequency Capping}
%          {After completing initial memory optimizations of Frequency Capping with our CTO, I reworked the entirety of this feature to use faster queries and significantly less memory. Additionally, this set the foundation to build highly-requested new sub-features rapidly.}
%        \resumeItem{Improvements to CI/CD Processes}
%          {Integrated CodeCov into our CI/CD process to surface test coverage information. Integrated Danger gem to surface Rubocop (code style) errors in GitHub Pull Requests. Changed CI Docker image to use Ramdisks for MongoDB to speed up tests. Reworked all CI test scripts to facilitate high paralellization.}
